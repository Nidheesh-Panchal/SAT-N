...
class TseitinTransformer
{
	private:
		...
		int transSubformula(TreeNode *root); // recursive function called on each subformula
	...
	public:
		...
		std::string cnfString(TreeNode *root) const; // a debug helper function to check the correctness of transformation
		unsigned int getVarNum() const;
		unsigned int checkVarNum(TreeNode *root);
};
